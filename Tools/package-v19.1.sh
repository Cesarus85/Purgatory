...
quest_export="${1:?Provide the completed V19.1 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v191-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.1$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=40$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v19.1-rhythm-life.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V19.1 delivery APK"; exit 1; fi
quest_stage="/private/tmp/qdmr-v184-package.SE7n42"
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v191-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.[01]$' "$quest_stage/gradle.properties"
fi
