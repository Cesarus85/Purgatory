...
quest_export="${1:?Provide the completed V19.13 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1913-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.13$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=52$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v19.13-divine-shotgun.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1913-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(12|13)$' "$quest_stage/gradle.properties"
fi
