...
quest_export="${1:?Provide the completed V18.7 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v187-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.18.7$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=25$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v18.7-audio.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V18.7 delivery APK"; exit 1; fi
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v187-package.XXXXXX)
else
  rg -q '^unity.versionName=0.18.[4567]$' "$quest_stage/gradle.properties"
fi
