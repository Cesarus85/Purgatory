...
quest_export="${1:?Provide the completed V18.17 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1817-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.18.17$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=35$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v18.17-scan-shaft.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V18.17 delivery APK"; exit 1; fi
quest_stage="/private/tmp/qdmr-v184-package.SE7n42"
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1817-package.XXXXXX)
else
  rg -q '^unity.versionName=0.18.([456789]|10|11|12|13|14|15|16|17)$' "$quest_stage/gradle.properties"
fi
