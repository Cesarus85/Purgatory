...
quest_export="${1:?Provide the completed V19.12 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1912-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.12$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=51$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v19.12-fluidity.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1912-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(11|12)$' "$quest_stage/gradle.properties"
fi
# Task-owned validated build workspace only; retain native compilation caches.
