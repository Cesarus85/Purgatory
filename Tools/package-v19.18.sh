...
quest_export="${1:?Provide the completed V19.18 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1918-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.18$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=57$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v19.18-creature-polish.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1918-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(17|18)$' "$quest_stage/gradle.properties"
fi
