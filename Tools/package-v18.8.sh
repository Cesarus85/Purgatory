...
quest_export="${1:?Provide the completed V18.8 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v188-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.18.8$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=26$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v18.8-enemy-audio.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V18.8 delivery APK"; exit 1; fi
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v188-package.XXXXXX)
else
  rg -q '^unity.versionName=0.18.[45678]$' "$quest_stage/gradle.properties"
fi
