...
quest_export="${1:?Provide the completed V18.6 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v186-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.18.6$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=24$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v18.6-revolver.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V18.6 delivery APK"; exit 1; fi
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v186-package.XXXXXX)
else
  rg -q '^unity.versionName=0.18.[456]$' "$quest_stage/gradle.properties"
fi
