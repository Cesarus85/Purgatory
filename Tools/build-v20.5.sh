...
test ! -e Builds/Purgatory-v20.5-katana-reliability.apk
bash Tools/quarantine-typedb-copies.sh
export QDMR_GRADLE_EXPORT
QDMR_GRADLE_EXPORT=$(mktemp -d /private/tmp/qdmr-v205-export.XXXXXX)
quest_log="$QDMR_GRADLE_EXPORT.unity.log"
echo "Unity log: $quest_log"
"/Applications/Unity/Hub/Editor/6000.3.2f1/Unity.app/Contents/MacOS/Unity" -batchmode -force-metal -quit -projectPath "$quest_project_root" -executeMethod QuestDemonMR.Editor.V205Validation.ValidateAndExport -logFile "$quest_log"
rg -q '^QDMR_V205_EXPORT_OK' "$quest_log"
if rg -q 'error CS[0-9]|Shader error|Exception:|TypeDB: Class' "$quest_log"; then exit 1; fi
quest_stage=$(mktemp -d /private/tmp/qdmr-v205-package.XXXXXX)
...
cp -n "$quest_stage/launcher/build/outputs/apk/release/launcher-release.apk" "$quest_project_root/Builds/Purgatory-v20.5-katana-reliability.apk"
