"""Run with project root and actual V20.6 export; safe to stage in /private/tmp."""
import hashlib,json,os,re,subprocess,sys,zipfile
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor
root=Path(sys.argv[1]);export=Path(sys.argv[2]);out=root/'Verification/V20.6'
assert str(export).startswith('/private/tmp/qdmr-v206-export.')
def sha(p):
...
normalized=settings.read_bytes().replace(b'bundleVersion: 0.20.6',b'bundleVersion: 0.20.4').replace(b'AndroidBundleVersionCode: 66',b'AndroidBundleVersionCode: 64')
...
apk=root/'Builds/Purgatory-v20.6-contact-portals.apk'
...
assert "versionCode='66'" in badging and "versionName='0.20.6'" in badging
...
with zipfile.ZipFile(apk) as z:
    ...
    for name in [b'DamageStrength',b'PortalExitSafety',b'ResolveEmission',b'ContactStrength']:assert name in metadata
...
assert sha(root/'Builds/Purgatory-v20.5-katana-reliability.apk')=='6eef6667de1548bec644c449d00ed7b17532716754046a23a1c9ecc9efd573a6'
...
assert 'QDMR_V206_EXPORT_OK' in text and 'QDMR_STARTUP_SERIALIZATION_OK' in text
assert not re.search(r'error CS\d|Shader error|Exception:|TypeDB: Class',text)
report=dict(apk=str(apk),bytes=apk.stat().st_size,sha256=sha(apk),version='0.20.6',code=66,certificate=cert,libil2cpp=lib,source_verified=True,source_files=len(expected),post_settings=sha(settings),settings_match_v204_plus_version=True,unity_log=str(log),native=json.loads((out/'native-result.json').read_text()),installed=False,headset_tested=False)
