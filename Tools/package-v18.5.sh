...
quest_export="${1:?Provide the completed V18.5 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v185-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.18.5$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=23$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v18.5-projectiles.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V18.5 delivery APK"; exit 1; fi
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v185-package.XXXXXX)
else
  rg -q '^unity.versionName=0.18.[45]$' "$quest_stage/gradle.properties"
fi
