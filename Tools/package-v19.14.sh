...
quest_export="${1:?Provide the completed V19.14 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1914-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.14$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=53$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v19.14-heaven-spread-entry.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1914-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(13|14)$' "$quest_stage/gradle.properties"
fi
# Only this validated task-owned generated checkout; retain compilation caches.
