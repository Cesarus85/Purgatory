...
quest_export="${1:?Provide the completed V18.9 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v189-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.18.9$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=27$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v18.9-shot-clarity.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V18.9 delivery APK"; exit 1; fi
# Reuse only this verified, task-owned temporary build checkout, keeping its
# native compiler cache. Prior immutable APKs and Unity exports are untouched.
quest_stage="/private/tmp/qdmr-v184-package.SE7n42"
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v189-package.XXXXXX)
else
  rg -q '^unity.versionName=0.18.[456789]$' "$quest_stage/gradle.properties"
fi
# Preserve Gradle/native build caches; synchronize only generated export inputs.
# --delete is bounded to this explicit temporary Gradle checkout, not workspace.
