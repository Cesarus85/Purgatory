...
quest_export="${1:?Provide the completed V19.17 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1917-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.17$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=56$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v19.17-weapons-portal.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1917-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(16|17)$' "$quest_stage/gradle.properties"
fi
