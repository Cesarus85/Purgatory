...
quest_export="${1:?Provide the completed V19.10 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1910-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.10$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=49$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/QuestDemonMR-v19.10-two-point.apk"
if [ -e "$quest_apk" ]; then echo "Refusing to overwrite V19.10 delivery APK"; exit 1; fi
quest_stage="/private/tmp/qdmr-v184-package.SE7n42"
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1910-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(9|10)$' "$quest_stage/gradle.properties"
fi
