...
quest_export="${1:?Provide completed V20.1 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v201-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.20.1$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=61$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v20.1-katana-fixes.apk"
test ! -e "$quest_apk"
quest_stage=$(mktemp -d /private/tmp/qdmr-v201-package.XXXXXX)
