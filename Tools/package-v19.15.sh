...
quest_export="${1:?Provide the completed V19.15 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1915-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.15$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=54$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v19.15-presence.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1915-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(14|15)$' "$quest_stage/gradle.properties"
fi
