...
quest_export="${1:?Provide the completed V19.16 Unity export}"
case "$quest_export" in /private/tmp/qdmr-v1916-export.*) ;; *) echo "Unexpected export path"; exit 1;; esac
rg -q '^unity.versionName=0.19.16$' "$quest_export/gradle.properties"
rg -q '^unity.versionCode=55$' "$quest_export/gradle.properties"
quest_apk="$quest_project_root/Builds/Purgatory-v19.16-stars-atmosphere.apk"
...
if [ ! -f "$quest_stage/gradle.properties" ]; then
  quest_stage=$(mktemp -d /private/tmp/qdmr-v1916-package.XXXXXX)
else
  rg -q '^unity.versionName=0.19.(15|16)$' "$quest_stage/gradle.properties"
fi
