"""Knee-led threshold stride: lower ankle lift and an upward knee hint."""
import bpy,math,shutil
from pathlib import Path
from mathutils import Matrix,Vector
ROOT=Path(__file__).resolve().parents[1];out=ROOT/'Verification/KneeDive';out.mkdir(parents=True,exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(ROOT/'BlenderSource/RiftStalkerV18_18.blend'))
...
source=source.replace('forward=Vector((0,-1,0))','forward=Vector((0,-1,.75))')
exec('def aim'+source.split('def aim',1)[1].split('def foot',1)[0])
def foot(side,t):
    pos=-.56
    steps=[(.02,.23,-.30,.08),(.35,.66,.50,.35),(.80,1,.54,.08)] if side=='L' else [(.15,.32,-.32,.08),(.62,.94,.54,.35)]
    for start,end,target,height in steps:
        if t>=end:pos=target
        elif t>start:
            p=(t-start)/(end-start);carry=max(0,min(1,(p-.18)/.44)) if side=='R' and height>.2 else p;s=carry*carry*(3-2*carry);v=math.sin(math.pi*p)
            def smooth(u):u=max(0,min(1,u));return u*u*(3-2*u)
            rise=(smooth(p/.34) if p<.34 else 1-smooth((p-.72)/.28)) if height>.2 else v*v*((3-2*v) if p<.5 else 1)
            return pos+(target-pos)*s,height*rise
        else:break
    return pos,0
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'BlenderSource/RiftStalkerV18_19.blend'))
...
print('QDMR_KNEE_BLENDER_OK steps=two lift=.35 preserved=Leap',flush=True)
